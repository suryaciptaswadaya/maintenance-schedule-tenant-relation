...
class SmsMailTemplateHashtagController extends Controller
{
    public function html(Request $request)
    {
        ...
        foreach ($mailTemplateHashTags as $mailTemplateHashTag) {
            ...
            $informationHtml .=         '<input type="text" name="mail_template_hashtag['.$mailTemplateHashTag->field.']" class="form-control information-template '.SmsMailTemplateHashtag::VALUE_TYPE_HTML_CLASS[$mailTemplateHashTag->value_type].'" id="'.$mailTemplateHashTag->field.'" placeholder="Masukkan '.$mailTemplateHashTag->title.'">';
            $informationHtml .=     '</div>';
            $informationHtml .= '</div>';
        }
